...
bool InstallHook() {
    ...
    if (!dm::CreateHook(g_hook_target, reinterpret_cast<void*>(&HookVoteUpdate),
                        reinterpret_cast<void**>(&g_original_vote_update))) {
        g_hook_target = nullptr;
...
void RemoveHook() {
    ...
    if (g_hook_created && g_hook_target != nullptr) {
        dm::QueueHook(g_hook_target, false);
        dm::ApplyHooks();
...
extern "C" __declspec(dllexport) BOOL WINAPI Mod_Initialize(const DmModHostContext* context) {
    if (!dm::AcceptHostContext(context, "dark_assembly", true)) return FALSE;
...
extern "C" __declspec(dllexport) BOOL WINAPI Mod_Enable() {
    if (!g_hook_created || g_hook_target == nullptr) return FALSE;
