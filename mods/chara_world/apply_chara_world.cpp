...
int main() {
    ...
    printf("=================================================================\n");
    printf("  DISGAEA MAYHEM - CHARA WORLD INFINITE ENERGY (C++ NATIVE)\n");
    printf("=================================================================\n");
    ...
    // Injetar chara_world.dll
    char dll_full_path[MAX_PATH] = {};
    GetFullPathNameA("chara_world.dll", MAX_PATH, dll_full_path, NULL);

    if (GetFileAttributesA(dll_full_path) == INVALID_FILE_ATTRIBUTES) {
        GetFullPathNameA("mods/chara_world/chara_world.dll", MAX_PATH, dll_full_path, NULL);
    }
    ...
    if (pRemoteMem) {
        ...
        if (hThread) {
            ...
            CloseHandle(hThread);
            printf("[SUCESSO] Hook residente do Chara World ATIVADO (ON) na memoria do jogo!\n");
            printf("  -> Energia travada em 100/100 para movimentos e batalhas infinitas.\n");
        } else {
